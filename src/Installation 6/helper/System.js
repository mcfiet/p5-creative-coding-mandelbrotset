...
export class System {
  ...
  update(distance, active = false) {
    const radius = 700; // Abstand von der Zentralsphäre
    if (active) {
      camera(
    ...
    for (let i = 0; i < this.orbiters.length; i++) {
      ...
      if (i < this.orbiters.length / 3) {
        // this.centralSphere.radius = map(distance[0], 50, 255, 50, 10);
        this.orbiters[i].col = color(150, 100, 100);
        this.orbiters[i].changeDistance(
          map(distance[0], 50, 200, 300, 50),
          150
        );
      } else if (i > this.orbiters.length / 3) {
        // this.centralSphere.radius = map(distance[0], 50, 255, 50, 10);
        this.orbiters[i].col = color(20, 100, 100);
        this.orbiters[i].changeDistance(
          map(distance[2], 50, 200, 300, 50),
          150
        );
      }
    ...
    // this.centralSphere.col = color(map(distance[1], 0, 200, 360, 0), 100, 100);
    this.attractor.color = color(10, 100, map(distance[0], 10, 255, 100, 0));
